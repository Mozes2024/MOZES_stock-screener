...
# 📊 מסנן מניות חכם (MOZES - Intelligent Stock Screener)

מערכת מסחר שיטתית ומתקדמת לאיתור סט־אפים בעלי הסתברות גבוהה במניות לפריצה תוך שימוש בניתוח טכני מבוסס־שלבים, מומנטום חוזק יחסי וסינון פונדמנטלי חכם.

---

## 🧠 סקירה כללית

המערכת אוטומטית לחלוטין, מונעת־נתונים ה **Intelligent Stock Screener** לאיתור מניות הכי שוות להשקעה - ** MOZES STOCK SCREENER **  
ההפלטפורמה סורקת מדי יום יותר מ־3,800 מניות בארה״ב ומפיקה אותות קנייה ומכירה מבוססי ניתוח טכני, פונדמנטלי וניהול סיכונים קפדני
כאשר כל הסקרינרים המובילים מסתמכים על מדדים פונדמנטליים סטטיים (P/E, P/B) או דפוסים טכניים גנריים שאינם מתאימים את עצמם לתנאי השוק. הם מתעלמים מ:
- **מצב השוק** (האם SPY נמצא במגמת עלייה או ירידה?)
- **חוזק יחסי** (האם המניה מציגה ביצועים טובים יותר מהשוק?)
- **מעברי פאזה** (האם המניה פורצת או קורסת?)
- **אחסון חכם במטמון** (קריאות API מיותרות מבזבזות זמן ועוברות גבולות קצב)

**הפלטפורמה הזו הכי טוב כי:** מדובר במערכת מתקדמת שמיישמת את **מתודולוגיית תבנית המגמה** של מארק מינרוויני כאשר:
- קנו רק **מגמות עלייה מאושרות בשלב 2** (לא בסיסים מוקדמים של שלב 1)
- חייבים לעמוד ב-**7 מתוך 8 קריטריונים טכניים מחמירים** (50>150>200 SMA, מחיר קרוב לשיאים וכו')
- **התזמון חשוב** - קנו רק כאשר השוק (SPY) בריא גם כן
- **חוזק יחסי חשוב** - קנו רק מניות מציגות ביצועים טובים יותר מהשוק
- **עיצוב מטמון תחילה** - הפחתה של 74% בקריאות API באמצעות אחסון בסיסי מבוסס Git.

המערכת נבנתה כדי:
- לבטל קבלת החלטות רגשית  
- לספק אותות מסחר עקביים וניתנים לשחזור  
- לעבוד בהתאם למטריצה של מתודולוגיות מוכחות (Minervini, O’Neil, Weinstein)

---

## ⚙️ מה המערכת עושה

1. סריקה יומית של 3,800+ מניות (NASDAQ / NYSE)  
2. זיהוי אותות קנייה בשלב מגמה 2 (Phase 2) מאושר  
3. זיהוי אותות מכירה בשלבי הפצה וירידה  
4. חישוב סטופ־לוס מדויק ויחס סיכון/סיכוי  
5. הפקת דוחות יומיים מדורגים  
6. ניהול פוזיציות עם המלצות לגרירת סטופ
...
- **שלב 1 – בסיס (Base)**  
  קונסולידציה לאחר ירידה. עדיין לא מוכן לכניסה.

- **שלב 2 – מגמת עלייה (Uptrend)**  
  מגמה מאושרת – אזור קנייה.

- **שלב 3 – הפצה (Distribution)**  
  היחלשות מומנטום – אזור מכירה.

- **שלב 4 – מגמת ירידה (Downtrend)**  
  מגמה שלילית – להימנע.

> 📌 המערכת מפיקה אותות קנייה **אך ורק** לשלב 2.

---

## 🧮 תבנית המגמה של Minervini (8 קריטריונים)

אות קנייה מופק רק אם המניה עוברת לפחות **7 מתוך 8** הקריטריונים:

✅ מחיר מעל SMA 150 ו־200  
✅ ממוצע נע 150 מעל ממוצע נע  200  
✅ ממוצע נע 200 בעל שיפוע עולה  
✅ ממוצע נע 50 גדול מ 150 שגדול מ 200 (SMA 50 > 150 > 200)
✅ מחיר נוכחי מעל ממוצע 50  
✅ לפחות 30% מעל השפל השנתי  
✅ עד 25% מתחת לשיא השנתי  
✅ חוזק יחסי גבוה (RS)

---**למה זה עובד:**
- מזהה מניות ב **מגמות עלייה מאושרות** (לא בסיסים מוקדמים)
- מסנן מניות חלשות עם מבנה מחירים גרוע
- מבטיח סיכון/תגמול מתאימים (מניות קרובות לשיאים עם מומנטום חזק)
- מתיישר עם קנייה מוסדית (כסף גדול עוקב אחר מגמות חזקות)

### השפעות נוספות

בעוד שקריטריוני הכניסה המרכזיים עוקבים אחר התבנית של מינרוויני, מערכת זו משלבת גם מושגים ממטודולוגיות "חכמות" נוספות במטריצה לרבות :

- **סטן ויינשטיין** - ניתוח מחזורי שוק בן 4 שלבים ("סודות לרווח בשווקים שווריים ודוביים")
- **ויליאם אוניל** - קריטריוני סינון בסיסיים (מתודולוגיית CANSLIM)
- **מתודולוגיית IBD** - דירוג חוזק יחסי ומודעות למשטר שוק

## 💪 חוזק יחסי (Relative Strength)

- חישוב מומנטום ל־63 ימים  
- דירוג לינארי חלק (0–10)  
- קנייה רק של מניות שמכות את SPY
...
- **צמיחה:** הכנסות, EPS  
- **רווחיות:** ROE, מרווחים  
- **חוסן פיננסי:** חוב, נזילות  

---

## 🛑 ניהול סיכונים וסטופ־לוס
...
- ריצה יומית באמצעות GitHub Actions  
- ללא התערבות ידנית  
- שמירת היסטוריה מלאה  
- התאמה אוטומטית למצב רוח של השוק

##  עיצוב טכנולוגי

### 1. **קודם כל מטמון, אחרי זה שלוף מה API**

**הסבר:** רוב הסקנרים מתבססים על נתונים שנשלפים מ-yfinance עם מגבלות קצב קפדניות (כ-2 קריאות לשנייה). סריקת 3,800 מניות דורשת יותר מ-3,800 קריאות API. ב-1 TPS, זה 63 דקות. אבל היסודות משתנים לאט (רווחים רבעוניים), אז למה לאחזר מחדש כל יום?

**הפתרון:**
- אחסן יסודות במאגר Git כקבצי JSON
- הוסף מטא-דאטה עם חותמת זמן `fetched_at`
- במהלך הסריקה: בדוק אם המטמון קיים והוא חדש
- אחזר רק אם הוא מיושן או חסר
- תוצאה: 1,762 מניות במטמון = **74% פחות קריאות API** = **סריקות מהירות יותר ב-15-20 דקות**
