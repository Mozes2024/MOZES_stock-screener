מסנן מניות חכם (Intelligent Stock Screener)
מערכת מסחר שיטתית ברמת פרודקשן לאיתור סט־אפים בעלי הסתברות גבוהה במניות, תוך שימוש בניתוח טכני מבוסס־שלבים, מומנטום חוזק יחסי וסינון פונדמנטלי חכם.

סקירה כללית
Intelligent Stock Screener היא מערכת אוטומטית לחלוטין, מונעת־נתונים, לסריקת מניות וניהול פוזיציות. המערכת סורקת מדי יום יותר מ־3,800 מניות בארה״ב כדי לזהות אותות קנייה ומכירה בעלי רמת ודאות גבוהה, בהתבסס על:

סיווג מגמה מבוסס שלבים (ניתוח מחזור שוק בן 4 שלבים)

מומנטום חוזק יחסי (דירוג לינארי חלק מול SPY)

מסנני איכות פונדמנטליים (צמיחה, רווחיות, חוסן פיננסי)

זיהוי פריצות משוקללות נפח

המלצות סטופ־לוס מנוהלות סיכון

המערכת נבנתה כדי לבטל קבלת החלטות רגשית ולספק אותות מסחר אובייקטיביים, עקביים וניתנים לשחזור, הנתמכים בניתוח טכני ופונדמנטלי.

מה המערכת עושה
סורקת יותר מ־3,800 מניות בכל יום מסחר באמצעות אוטומציית GitHub Actions

מזהה אותות קנייה למניות הנמצאות במגמת עלייה מאושרת (Phase 2) שעוברות את תבנית המגמה של מינרויני (8 קריטריונים)

מזהה אותות מכירה כאשר מניות נכנסות לשלב 3 (הפצה) או שלב 4 (מגמת ירידה) עם היחלשות מומנטום

מחשבת סטופ־לוס מדויק עם יחס סיכון/סיכוי לכל אות קנייה (סיכון מקסימלי 10%, מינימום יחס 2:1)

מפיקה דוחות יומיים עם הזדמנויות מדורגות וניתוח טכני מלא

מנהלת פוזיציות קיימות עם המלצות אוטומטיות לגרירת סטופ־לוס

למה בניתי את זה כך
הבעיה: רוב מסנני המניות נשענים על מדדים פונדמנטליים סטטיים (כמו P/E, P/B) או תבניות טכניות כלליות שאינן מסתגלות לתנאי השוק. הם מתעלמים מ־

משטר השוק (האם SPY במגמת עלייה או ירידה?)

חוזק יחסי (האם המניה מנצחת את השוק?)

מעברי שלבים (פריצה או שבירה?)

קאשינג חכם (קריאות API מיותרות הפוגעות בזמן ובמגבלות קצב)

הפתרון: יישום מתודולוגיית Trend Template של מארק מינרויני, לפיה:

קונים רק מגמות עלייה מאושרות בשלב 2 (לא בסיסים מוקדמים בשלב 1)

חובה לעבור 7 מתוך 8 קריטריונים טכניים מחמירים (50>150>200 SMA, מחיר קרוב לשיאים וכו’)

תזמון חשוב – קנייה רק כאשר גם השוק (SPY) בריא

חוזק יחסי חשוב – קנייה רק של מניות שמכות את השוק

עיצוב Cache-First – הפחתה של 74% בקריאות API באמצעות אחסון פונדמנטלי מבוסס Git

מתודולוגיה וייחוס
המערכת מיישמת את Trend Template של מארק מינרויני (ידוע גם כ־SEPA – Specific Entry Point Analysis), גישה שיטתית לאיתור סט־אפים בעלי הסתברות גבוהה במגמות עלייה מאושרות (Stage 2).

השפעת מארק מינרויני
שיטת אות הקנייה מבוססת על עקרונות מספריו:

Trade Like a Stock Market Wizard (2013)

Think & Trade Like a Champion (2017)

Momentum Masters (2015)

תבנית המגמה של מינרויני – 8 קריטריונים
אותות קנייה מופעלים רק כאשר המניה עוברת 7 מתוך 8 הקריטריונים הבאים:

מחיר > ממוצע נע 150 ו־200 – מגמת עלייה מבוססת

ממוצע נע 150 > ממוצע נע 200 – יישור שורי

ממוצע נע 200 בעל שיפוע עולה ≥ חודש

50 > 150 > 200 – תצורת SMA החזקה ביותר

מחיר > ממוצע נע 50

מחיר ≥ 30% מעל השפל השנתי

מחיר בתוך 25% מהשיא השנתי

חוזק יחסי ≥ 70 (אצלנו: שיפוע RS ≥ ‎+0.15)

למה זה עובד
מזהה מניות במגמות עלייה מאושרות

מסנן מניות חלשות

מבטיח יחס סיכון/סיכוי תקין

מתיישר עם קניות מוסדיות

השפעות נוספות
סטאן ויינשטיין – ניתוח מחזור שוק בן 4 שלבים

ויליאם אוניל – קריטריונים פונדמנטליים (CANSLIM)

מתודולוגיית IBD – דירוג חוזק יחסי ומודעות לשוק

המימוש שלנו
אוטומציה של תהליך הסריקה הידני של מינרויני עם שיפורים:

ניקוד לינארי חלק במקום כן/לא

סינון משטר שוק (שלב SPY + רוחב שוק)

סינון פונדמנטלי אוטומטי

חישוב סטופ־לוס מדויק (ATR + שפלים)

אימות יחס סיכון/סיכוי (≥ 2:1)

המערכת נשארת נאמנה לעיקרון: קנייה של פריצות Stage 2 מאושרות עם פונדמנטליים חזקים וניהול סיכון ברור.

ארכיטקטורת המערכת
זרימת נתונים
סריקה יומית (GitHub Actions, ‎13:00 UTC, ימים א׳–ה׳)

משיכת יקום של 3,800+ מניות NASDAQ/NYSE

הורדת היסטוריית מחירים לשנה

משיכת פונדמנטליים עם ריענון חכם

אחסון בקאש מבוסס Git
...
סיווג שלב, RS ופונדמנטליים לכל מניה

דירוג והפקת דוח

ניהול פוזיציות (ידני)

קריאה בלבד מ־Robinhood

ניתוח פוזיציות והמלצות סטופ

דוח עם מחירי יעד מדויקים

תכונות עיקריות
סיווג מגמה מבוסס שלבים
שלב 1 – בסיס: קונסולידציה, לא מוכן

שלב 2 – מגמת עלייה: אזור קנייה

שלב 3 – הפצה: אזור מכירה

שלב 4 – מגמת ירידה: להימנע

אותות קנייה רק לשלב 2 שעובר Trend Template.

מומנטום חוזק יחסי
ניקוד לינארי 0–10 מול SPY

שיפוע RS ל־63 ימים

קנייה רק של מניות שמכות את השוק

סינון פונדמנטלי
צמיחה, רווחיות, חוסן פיננסי

מיושם רק לשלבים 1–2

סטופ־לוס מנוהל סיכון
מחיר כניסה

סטופ מבוסס ATR או שפל

יעד רווח מינימום פי 2 מהסיכון

אסטרטגיית קאשינג חכמה
מחירים: תמיד רענון

פונדמנטליים: אחסון Git עם ריענון לפי עונת דוחות

תוצאה: 74% פחות קריאות API

פילוסופיית עיצוב
Cache First

רק מגמות עלייה מאושרות

חוזק יחסי כמסנן ראשי

נוסחאות לינאריות (לא דליים)

מודעות למשטר השוק

ניהול פוזיציות בקריאה בלבד
...
pip install robin-stocks  # אופציונלי
הרצה:

python run_optimized_scan.py --test-mode
python run_optimized_scan.py --conservative
לוגיקת סטופ־לוס (סיכום)
נוסחאות לינאריות לפי אחוז רווח

נעילת רווח הדרגתית

יחס סיכון/סיכוי מינימום 2:1

פיתוחים עתידיים
טווח קצר: Backtesting, דוחות משופרים, התראות
טווח בינוני: ניתוח רב־טיימפריים, סקטורים, אופציות
טווח ארוך: ML, דאטה אלטרנטיבי, בניית תיק, אופטימיזציית ביצוע
תשתית: DB, דאטה בזמן אמת, Web UI

תרומה לפרויקט
הפרויקט אישי. ניתן לתרום דרך Fork ו־Pull Request.
סטנדרטים: Python 3.13+, Type hints, pytest, Black.

רישיון והצהרת אחריות
רישיון MIT
למטרות לימודיות בלבד.
לא ייעוץ פיננסי.
מסחר כרוך בסיכון.
...
**הסקריפט run_optimized_scan.py הוא מערכת ניתוח כמותית שנועדה לסרוק אלפי מניות בשוק האמריקאי, תוך שימוש במודל ניתוח טכני ופונדמנטלי חזק (ככל הנראה בהשראת עקרונות קנזלים - CAN SLIM). 
מטרתו היא לזהות מניות שנמצאות בשלבי עלייה חזקים ומציגות סימני מומנטום, איכות ופריצה, תוך התחשבות בבריאות השוק הכללית.
הסקריפט מחשב ומנתח נתונים קריטיים עבור כל מניה:מדד מהות הניתוח חשיבות Phase Analysis קביעת השלב המחזורי של המניה (1, 2, 3 או 4) לפי המודל של Stan Weinstein (יצירת מגמה).
מניה חייבת להיות בשלב 1 או 2 כדי לקבל ציון קנייה.RS (Relative Strength) מדד מומנטום שמשווה את ביצועי המניה ל-SPY/שוק הכללי.
מדד מרכזי לזיהוי מניות ש"מכות את השוק" (מומנטום חזק).VCP (Volatility Contraction Pattern) ניתוח דפוסי ירידה בתנודתיות המחיר, המעיד על הצטברות לקראת פריצה (ככל הנראה בהשראת Mark Minervini).ציון איכות נוסף לפריצות מאומתות.R/R Ratio (Risk/Reward) חישוב יחס הסיכון-סיכוי של עסקת הקנייה המומלצת.קביעת איכות העסקה; מועדף יחס של 2:1 ומעלה.Fundamental Snapshotבדיקת נתוני יסוד (כגון רווחים, הכנסות) כמדד לאיכות החברה (תוך שימוש ב-FMP אם הוגדר).מוסיף ציון איכות לאיתות קנייה
