...
def score_buy_signal(
    ...
    # Liquidity / Dollar-Volume penalty (execution feasibility)
    # 20-day average dollar volume = mean(Close * Volume) over the last 20 sessions.
    avg_dollar_volume = 0.0
    liquidity_penalty = 0.0
    try:
        if 'Volume' in price_data.columns and 'Close' in price_data.columns and len(price_data) >= 5:
            adv_series = (price_data['Close'] * price_data['Volume']).tail(20)
            avg_dollar_volume = float(adv_series.mean()) if not adv_series.empty else 0.0

        # Penalty bands (USD/day):
        # < $2M      : -10 (very hard to execute reliably)
        # $2M–$10M   : -10 down to -2 (linear)
        # $10M–$20M  : -2 down to 0 (linear)
        if avg_dollar_volume > 0:
            if avg_dollar_volume < 2_000_000:
                liquidity_penalty = 10.0
            elif avg_dollar_volume < 10_000_000:
                liquidity_penalty = 10.0 - (avg_dollar_volume - 2_000_000) * (8.0 / 8_000_000)
            elif avg_dollar_volume < 20_000_000:
                liquidity_penalty = 2.0 - (avg_dollar_volume - 10_000_000) * (2.0 / 10_000_000)
            else:
                liquidity_penalty = 0.0
        ...
        details['avg_dollar_volume_20d'] = round(avg_dollar_volume, 2)
        details['liquidity_penalty'] = round(liquidity_penalty, 2)

        if liquidity_penalty >= 6:
            reasons.append(f"⚠️ Low liquidity: ${avg_dollar_volume/1_000_000:.1f}M avg $ volume/day (penalty {liquidity_penalty:.1f})")
    except Exception as e:
        # Never fail the scan due to liquidity calc
        details['liquidity_error'] = str(e)
